CREATE TABLE field (
    id INTEGER PRIMARY KEY,
    routeid INTEGER DEFAULT NULL ,
    adminrouteid INTEGER DEFAULT NULL,
    parentid INTEGER DEFAULT NULL,
    label TEXT NOT NULL DEFAULT "unlabeled",
    data TEXT NOT NULL DEFAULT "",
    type TEXT NOT NULL DEFAULT "text",
    author TEXT NOT NULL DEFAULT "system",
    authorid INTEGER NOT NULL DEFAULT 1,
    datecreated TEXT NOT NULL DEFAULT LOCALTIMESTAMP,
    datemodified TEXT DEFAULT LOCALTIMESTAMP,
    FOREIGN KEY (author) REFERENCES user(username) ON DELETE SET DEFAULT ON UPDATE CASCADE,
    FOREIGN KEY (authorid) REFERENCES user(id) ON DELETE SET DEFAULT ON UPDATE CASCADE,
    FOREIGN KEY (routeid) REFERENCES route(id) ON DELETE SET DEFAULT ON UPDATE CASCADE,
    FOREIGN KEY (adminrouteid) REFERENCES adminroute(id) ON DELETE SET DEFAULT ON UPDATE CASCADE,
    FOREIGN KEY (parentid) REFERENCES datatype(id) ON DELETE SET DEFAULT ON UPDATE CASCADE
);
