...
plugin_info = {
    ...
    -- TUI screens (standalone pages in SSH TUI sidebar)
    screens = {                             -- optional
        { name = "main", label = "My Plugin", icon = "list" },
        { name = "edit", label = "Edit", hidden = true },
    },
...
**Screen entries**: `name` maps to `screens/<name>.lua`, `label` shown in sidebar, `hidden` screens reachable only via `navigate` action.

**Interface entries**: `name` maps to `interfaces/<name>.lua`, `mode` is `"inline"` (renders within field row) or `"overlay"` (full-screen modal).
