...
**Three-pool design**:
- **General channel** (`VMPool`): serves HTTP requests (100ms acquire timeout)
- **Reserved channel** (`VMPool`): serves hooks (guaranteed availability)
- **UI pool** (`UIVMPool`): serves TUI screen/field coroutines (no acquire timeout, held for session lifetime)

**Checkout lifecycle (general/reserved)**:
...
**Checkout lifecycle (UI)**:
1. `Get()`: non-blocking acquire (returns ErrPoolExhausted immediately if full)
2. VM held for lifetime of screen/field session
3. `Put()` or `CloseVM()` when coroutine finishes

UI VMs are separate from the standard pool. Default 4 UI VMs per plugin (`MaxUIVMs`). Pool exhaustion shows "Plugin busy" instead of blocking. Standard pool drain does not affect UI VMs.
