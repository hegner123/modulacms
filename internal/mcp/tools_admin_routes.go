...
func registerAdminRouteTools(srv *server.MCPServer, backend AdminRouteBackend) {
	...
	srv.AddTool(mcp.NewTool("admin_get_route_by_slug", mcp.WithDescription("Get a single admin route by URL slug. Unlike get_route which uses ID-based lookup, admin routes use slug-based lookup."), mcp.WithString("slug", mcp.Required(), mcp.Description("Admin route slug (e.g. '/admin')"))), handleAdminGetRoute(backend))
